...
import '../parser/operation_naming.dart';
...
NameRegistry buildNameRegistry(SpecDocument document, {OneOfPlan? oneOfPlan}) {
  ...
  //
  // Only "nameable" schema kinds (Object/Enum/OneOf/AllOf) that are truly
  // inline (schema.name == null, i.e. not a $ref to an existing component)
  // get a synthetic class name here. Array/Primitive bodies don't need a
  // wrapper class — the service generator resolves those directly (e.g.
  // `List<Foo>`) — and $ref bodies already have a registered name from the
  // component-schema pass above.
  //
  // [OpenApiParser] inserts the exact same set of schemas into
  // [SpecDocument.schemas] under these same computed names, so
  // [ModelGenerator] actually emits a file for every name registered here.
  bool isNameable(SchemaObject schema) =>
      schema.name == null &&
  ...
  for (final op in document.operations) {
    ...
    if (requestSchema != null && isNameable(requestSchema)) {
    ...
    for (final response in op.responses.values) {
      ...
      if (responseSchema != null && isNameable(responseSchema)) {
        final suffix = responseSuffix(response.statusCode);
