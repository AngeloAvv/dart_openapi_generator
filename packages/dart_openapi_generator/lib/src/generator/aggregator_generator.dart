...
/// The barrel `generated.dart` is emitted by [FileWriter] after it has the
/// complete merged file map.
///
/// Usage:
/// ```dart
/// final generator = AggregatorGenerator(registry);
/// final files = generator.generate(doc, resolved);
/// ```
final class AggregatorGenerator {
  ...
  Map<String, String> generate(
    ...
    return {
      'api_client.dart': emitLibrary(_buildLibrary(doc, tags, annotation)),
    };
  }
  ...
  Library _buildLibrary(
    ...
    return Library((lib) {
      ...
      lib.body.add(_buildClass(doc, sortedTags, annotation));
    });
  }
  ...
  Class _buildClass(
    ...
    final escaped = escapeKeyword(annotation.clientName);
    if (escaped != annotation.clientName) {
      onWarning?.call(
        'Client name "${annotation.clientName}" is a Dart keyword; '
        'escaped to "$escaped"',
      );
    }
    return Class((c) {
      c.docs.add('/// Generated API client for ${annotation.clientName}.');
      c.name = escaped;
